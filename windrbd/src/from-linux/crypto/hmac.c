...
{
	/* Not used in WinDRBD */
	printk("hmac_create function called, shouldn't do that.\n");
	return -EINVAL;
}

static int hmac_create_with_name(struct crypto_template *tmpl, const char *name)
{
...
int __init hmac_module_init(void)
{
	...
	err = crypto_register_template(&hmac_tmpl);
	if (err) {
		printk("crypto_register_template returned %d this isn't good.\n", err);
		return err;
	}
